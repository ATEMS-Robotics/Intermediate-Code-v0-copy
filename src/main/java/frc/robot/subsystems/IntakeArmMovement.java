...
public class IntakeArmMovement extends SubsystemBase {
   ...
   private static final double SLOW_SPEED = 0.05;
   private static final double MOTOR_ROTATIONS_TO_ARM_ROTATIONS = 1/8.4;
    ...
    public Command moveArmUp() {
        return run(() -> {
            ...
            double distanceToTop = Math.max(1, (TOP_POSITION - position));
            double speed = NORMAL_SPEED * (distanceToTop / SLOW_ZONE); 
            armMotor.setControl(new DutyCycleOut(speed));
            speed = Math.max(SLOW_SPEED, speed);
            }
    ...
    public void periodic() {
        if (!armLimitSwitch.get()) {
            ...
            System.out.println("Arm Degrees:" +getArmDegrees());
        }
    }
